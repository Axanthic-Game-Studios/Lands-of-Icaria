...
public class SpellFreezing extends AbstractSpell {
	...
	public void drinkConcoction(ItemStack stack, World worldIn, EntityLivingBase entityLiving) {
		entityLiving.addPotionEffect(new PotionEffect(Resources.frozenEffect, 400));
	}
	...
	public void spellHit(RayTraceResult result, World worldIn) {
		...
		} else if (result.typeOfHit.equals(RayTraceResult.Type.BLOCK) && worldIn.isSideSolid(result.getBlockPos(), EnumFacing.UP) && worldIn.isAirBlock(result.getBlockPos().up())) {
			worldIn.setBlockState(result.getBlockPos().up(), Blocks.SNOW_LAYER.getDefaultState());
		}
	}
}
