...
public class ToolBident extends ItemTool implements IItemCustomReach {
	...
	public String getItemStackDisplayName(ItemStack stack) {
		if (I18n.canTranslate("item.bident_" + toolMaterial.name().substring(ModInformation.ID.length() + 1) + ".name"))
			return I18n.translateToLocal("item.bident_" + toolMaterial.name().substring(ModInformation.ID.length() + 1) + ".name");
		try {
