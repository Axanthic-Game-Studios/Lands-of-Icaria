...
public class ItemSignPopulus extends ItemSign
{
	public ItemSignPopulus()
	{
		this.setMaxStackSize(16);
		this.setRegistryName("sign_populus");
		this.setUnlocalizedName("sign_populus");
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float x, float y, float z)
	{
		...
		else
		{
			...
			if (!player.canPlayerEdit(pos, facing, stack) || !LOISigns.signStandingPopulus.canPlaceBlockAt(world, pos))
			{
				return EnumActionResult.PASS;
			}
			...
			else
			{
				if (facing == EnumFacing.UP && !player.isSneaking())
				{
					...
					world.setBlockState(pos, LOISigns.signStandingPopulus.getDefaultState().withProperty(BlockSignStandingPopulus.ROTATION, rotation), 3);
				}
				...
				else
				{
					...
					world.setBlockState(pos, LOISigns.signWallPopulus.getDefaultState().withProperty(BlockSignWallPopulus.ROTATION, finalRotation), 3);
				}

				TileEntity tileentity = world.getTileEntity(pos);

				if (tileentity instanceof TESignPopulus)
				{
