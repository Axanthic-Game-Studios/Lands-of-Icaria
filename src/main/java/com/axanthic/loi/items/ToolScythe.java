...
public class ToolScythe extends ItemSword implements IItemCustomReach {
	...
	public String getItemStackDisplayName(ItemStack stack) {
		if (I18n.canTranslate("item.scythe_" + material.material.name().substring(ModInformation.ID.length() + 1) + ".name"))
			return I18n.translateToLocal("item.scythe_" + material.material.name().substring(ModInformation.ID.length() + 1) + ".name");
		try {
