...
public class ItemSignPlane extends ItemSign
{
	public ItemSignPlane()
	{
		this.setMaxStackSize(16);
		this.setRegistryName("sign_plane");
		this.setUnlocalizedName("sign_plane");
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float x, float y, float z)
	{
		...
		else
		{
			...
			if (!player.canPlayerEdit(pos, facing, stack) || !LOISigns.signStandingPlane.canPlaceBlockAt(world, pos))
			{
				return EnumActionResult.PASS;
			}
			...
			else
			{
				if (facing == EnumFacing.UP && !player.isSneaking())
				{
					...
					world.setBlockState(pos, LOISigns.signStandingPlane.getDefaultState().withProperty(BlockSignStandingPlane.ROTATION, rotation), 3);
				}
				...
				else
				{
					...
					world.setBlockState(pos, LOISigns.signWallPlane.getDefaultState().withProperty(BlockSignWallPlane.ROTATION, finalRotation), 3);
				}

				TileEntity tileentity = world.getTileEntity(pos);

				if (tileentity instanceof TESignPlane)
				{
