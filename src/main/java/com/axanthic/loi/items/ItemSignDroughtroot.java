...
public class ItemSignDroughtroot extends ItemSign
{
	public ItemSignDroughtroot()
	{
		this.setMaxStackSize(16);
		this.setRegistryName("sign_droughtroot");
		this.setUnlocalizedName("sign_droughtroot");
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float x, float y, float z)
	{
		...
		else
		{
			...
			if (!player.canPlayerEdit(pos, facing, stack) || !LOISigns.signStandingDroughtroot.canPlaceBlockAt(world, pos))
			{
				return EnumActionResult.PASS;
			}
			...
			else
			{
				if (facing == EnumFacing.UP && !player.isSneaking())
				{
					...
					world.setBlockState(pos, LOISigns.signStandingDroughtroot.getDefaultState().withProperty(BlockSignStandingDroughtroot.ROTATION, rotation), 3);
				}
				...
				else
				{
					...
					world.setBlockState(pos, LOISigns.signWallDroughtroot.getDefaultState().withProperty(BlockSignWallDroughtroot.ROTATION, finalRotation), 3);
				}

				TileEntity tileentity = world.getTileEntity(pos);

				if (tileentity instanceof TESignDroughtroot)
				{
