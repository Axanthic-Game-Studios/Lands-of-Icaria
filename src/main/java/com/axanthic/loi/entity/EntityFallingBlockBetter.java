...
public class EntityFallingBlockBetter extends EntityFallingBlock {//just a big copy paste but with public variables
	...
	protected static final DataParameter<BlockPos> ORIGIN = EntityDataManager.<BlockPos>createKey(EntityFallingBlockBetter.class, DataSerializers.BLOCK_POS);
	protected static final DataParameter<NBTTagCompound> NBT = EntityDataManager.<NBTTagCompound>createKey(EntityFallingBlockBetter.class, DataSerializers.COMPOUND_TAG);

	public EntityFallingBlockBetter(World worldIn) {
		super(worldIn);
	}

	public EntityFallingBlockBetter(World worldIn, double x, double y, double z, IBlockState fallingBlockState) {
		super(worldIn);
		...
		if (this.hurtEntities) {
			...
			if (i > 0) {
				...
				if (flag && (double)this.rand.nextFloat() < 0.05000000074505806D + (double)i * 0.05D) {
					int j = ((Integer)this.getBlock().getValue(BlockAnvil.DAMAGE)).intValue();
					++j;

					if (j > 2) {
						this.dontSetBlock = true;
					} else {
						this.fallTile = this.getBlock().withProperty(BlockAnvil.DAMAGE, Integer.valueOf(j));
					}
				}
			}
		}
