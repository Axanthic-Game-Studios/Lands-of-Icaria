...
public class EntityLight extends EntityFlying {
	...
	AxisAlignedBB renderBox;// = new AxisAlignedBB(this.posX - d0, this.posY, this.posZ - d0, this.posX + d0, this.posY + (double)this.height, this.posZ + d0)

	public EntityLight(World worldIn) {
		super(worldIn);
		this.setSize(1.5F, 1.5F);
		//this.setRenderSize(10.0F, 10.0F);
		this.rand.setSeed((long)(1 + this.getEntityId()));
		this.rotationVelocity = 1.0F / (this.rand.nextFloat() + 1.0F) * 0.2F;
	}

	protected void initEntityAI() {
		//this.tasks.addTask(0, new EntityLight.AIMoveRandom(this));
	}
	...
	/*protected void setRenderSize(float width, float height)
    {
        if (width != this.renderWidth || height != this.renderHeight)
        {
            ...
            if (this.renderWidth < f)
            {
                double d0 = (double)width / 2.0D;
                renderBox = new AxisAlignedBB(this.posX - d0, this.posY, this.posZ - d0, this.posX + d0, this.posY + (double)this.renderHeight, this.posZ + d0);
                return;
            }

            AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
            renderBox = new AxisAlignedBB(axisalignedbb.minX, axisalignedbb.minY, axisalignedbb.minZ, axisalignedbb.minX + (double)this.renderWidth, axisalignedbb.minY + (double)this.renderHeight, axisalignedbb.minZ + (double)this.renderWidth);

            if (this.renderWidth > f && !this.firstUpdate && !this.world.isRemote)
            {
                this.move(MoverType.SELF, (double)(f - this.renderWidth), 0.0D, (double)(f - this.renderWidth));
            }
        }
    }
	...
	static class AIMoveRandom extends EntityAIBase {
		private final EntityLight jellyfish;

		public AIMoveRandom(EntityLight entityJellyfish) {
