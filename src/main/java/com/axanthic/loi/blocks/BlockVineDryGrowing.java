...
public class BlockVineDryGrowing extends BlockVine
{
    public BlockVineDryGrowing(String name)
    {
    ...
    {
        if (!worldIn.isRemote && worldIn.getBlockState(pos.up()).getBlock() != LOIVines.vineDry)
        {
                ...
                if(rand.nextBoolean())
                {
                    if(worldIn.isAirBlock(pos.down()) && pos.getY() > 0)
                    {
                        ...
                        while(worldIn.getBlockState(pos).getBlock() == this)
                        {
                            ...
                            if(rand.nextInt(8) == 0)
                            {
                                worldIn.setBlockState(pos, LOIVines.vineDryDead.getDefaultState()
                            ...
                            {
                                worldIn.setBlockState(pos, LOIVines.vineDry.getDefaultState()
                                        .withProperty(NORTH, state.getValue(NORTH))
                                        .withProperty(EAST, state.getValue(EAST))
                                        .withProperty(SOUTH, state.getValue(SOUTH))
                                        .withProperty(WEST, state.getValue(WEST)));
