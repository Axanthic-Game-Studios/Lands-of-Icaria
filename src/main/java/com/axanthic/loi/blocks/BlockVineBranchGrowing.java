...
public class BlockVineBranchGrowing extends BlockVine
{
    public BlockVineBranchGrowing(String name)
    {
                ...
                if(rand.nextBoolean())
                {
                    if(worldIn.isAirBlock(pos.down()) && pos.getY() > 0)
                    {
                        ...
                        while(worldIn.getBlockState(pos).getBlock() == this)
                        {
                            ...
                            if(rand.nextInt(8) == 0)
                            {
                                worldIn.setBlockState(pos, LOIVines.vineBranchDead.getDefaultState()
                            ...
                            {
                                worldIn.setBlockState(pos, LOIVines.vineBranch.getDefaultState()
                                        .withProperty(NORTH, state.getValue(NORTH))
                                        .withProperty(EAST, state.getValue(EAST))
                                        .withProperty(SOUTH, state.getValue(SOUTH))
                                        .withProperty(WEST, state.getValue(WEST)));
