...
public class BlockVineBloomyGrowing extends BlockVine
{
    public BlockVineBloomyGrowing(String name)
    {
                ...
                if(rand.nextBoolean())
                {
                    if(worldIn.isAirBlock(pos.down()) && pos.getY() > 0)
                    {
                        ...
                        while(worldIn.getBlockState(pos).getBlock() == this)
                        {
                            ...
                            if(rand.nextInt(8) == 0)
                            {
                                worldIn.setBlockState(pos, LOIVines.vineBloomyDead.getDefaultState()
