...
public class BlockVineReedyGrowing extends BlockVine
{
    public BlockVineReedyGrowing(String name)
    {
    ...
    {
        if (!worldIn.isRemote && worldIn.getBlockState(pos.up()).getBlock() != LOIVines.vineReedy)
        {
                ...
                if(rand.nextBoolean())
                {
                    if(worldIn.isAirBlock(pos.down()) && pos.getY() > 0)
                    {
                        ...
                        while(worldIn.getBlockState(pos).getBlock() == this)
                        {
                            ...
                            if(rand.nextInt(8) == 0)
                            {
                                worldIn.setBlockState(pos, LOIVines.vineReedyDead.getDefaultState()
                            ...
                            {
                                worldIn.setBlockState(pos, LOIVines.vineReedy.getDefaultState()
                                        .withProperty(NORTH, state.getValue(NORTH))
                                        .withProperty(EAST, state.getValue(EAST))
                                        .withProperty(SOUTH, state.getValue(SOUTH))
                                        .withProperty(WEST, state.getValue(WEST)));
