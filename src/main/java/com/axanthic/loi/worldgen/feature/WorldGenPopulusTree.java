...
public class WorldGenPopulusTree extends WorldGenLOITree {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		int heightTotal = heightTrunk + heightBottom + heightMiddle + 4;
		boolean flag = true;
		...
		if (position.getY() >= 1 && position.getY() + heightTotal + 1 <= 256) {
			...
			if (!flag) {
				...
				if (isSoil && position.getY() < worldIn.getHeight() - heightTotal - 1) {
					...
					this.placeLeafAtChance(worldIn, down.up(height).north(2), 2);
					this.placeLeafAtChance(worldIn, down.up(height).east(2), 2);
					this.placeLeafAtChance(worldIn, down.up(height).south(2), 2);
					this.placeLeafAtChance(worldIn, down.up(height).west(2), 2);
