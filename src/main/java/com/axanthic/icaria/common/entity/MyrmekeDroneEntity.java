...
public class MyrmekeDroneEntity extends Monster {
    public static EntityDataAccessor<Byte> CLIMBING = SynchedEntityData.defineId(MyrmekeDroneEntity.class, EntityDataSerializers.BYTE);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pSpawnType, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pTag) {
        ...
        if (pSpawnData == null) {
            pSpawnData = new MyrmekeDroneEffectsGroupData();
            if (pLevel.getDifficulty() == Difficulty.HARD && randomSource.nextFloat() < 0.1F * pDifficulty.getSpecialMultiplier()) {
                ((MyrmekeDroneEffectsGroupData)pSpawnData).setRandomEffect(randomSource);
            }
        }

        if (pSpawnData instanceof MyrmekeDroneEffectsGroupData) {
            MobEffect mobEffect = ((MyrmekeDroneEffectsGroupData)pSpawnData).mobEffect;
            if (mobEffect != null) {
    ...
    public static class MyrmekeDroneEffectsGroupData implements SpawnGroupData {
