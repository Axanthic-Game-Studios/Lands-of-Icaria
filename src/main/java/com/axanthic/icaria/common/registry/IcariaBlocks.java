...
public class IcariaBlocks {
	...
	public static class StoneDecoBlocks {
		...
		public StoneDecoBlocks(String name, RegistryObject<Block> original, Properties prop, boolean slab, boolean stairs, boolean wall) {
			this.name = name;
		...
		public StoneDecoBlocks(String name, RegistryObject<Block> original, Properties prop) {
	...
	public static class WoodDecoBlocks {
		...
		public WoodDecoBlocks(String name, RegistryObject<Block> original, Properties prop, boolean slab, boolean stairs, boolean fence, boolean gate) {
			this.name = name;
			this.original = original;
			if (slab)
				SLAB = register(name + "_slab", () -> new SlabBlock(prop));
			...
			if (stairs)
				...
				STAIRS = null;
			if (fence)
				FENCE = register(name + "_fence", () -> new FenceBlock(prop));
			else
				FENCE = null;
			if (gate)
				GATE = register(name + "_fence_gate", () -> new FenceGateBlock(prop));
			else
				GATE = null;
		}

		public WoodDecoBlocks(String name, RegistryObject<Block> original, Properties prop) {
			this(name, original, prop, true, true, true, true);
		}
	}
}
