...
public class SmallOliveTreeFeature extends SmallIcariaTreeFeature {
	public SmallOliveTreeFeature(Codec<NoneFeatureConfiguration> pCodec) {
		super(pCodec, IcariaBlocks.FALLEN_OLIVE_LEAVES.get(), IcariaBlocks.OLIVE_LEAVES.get(), IcariaBlocks.OLIVE_LOG.get());
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> pFeaturePlaceContext) {
		...
		if (level.getBlockStates(aabb).allMatch(BlockBehaviour.BlockStateBase::isSolidRender) && level.getBlockStates(aabb.move(0.0D, 1.0D, 0.0D)).allMatch(BlockBehaviour.BlockStateBase::isAir)) {
			...
			this.placeFallen(level, origin.north().east(), random.nextIntBetweenInclusive(4, 8), 2);
			this.placeFallen(level, origin.east().south(), random.nextIntBetweenInclusive(4, 8), 2);
			this.placeFallen(level, origin.south().west(), random.nextIntBetweenInclusive(4, 8), 2);
			this.placeFallen(level, origin.west().north(), random.nextIntBetweenInclusive(4, 8), 2);
