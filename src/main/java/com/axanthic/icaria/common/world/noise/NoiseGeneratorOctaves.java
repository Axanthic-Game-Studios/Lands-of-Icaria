...
public class NoiseGeneratorOctaves extends NoiseGenerator {
	...
	public double[] generateNoiseOctaves(double[] noiseArray, final int xOffset, final int yOffset, final int zOffset,
			final int xSize, final int ySize, final int zSize, final double xScale, final double yScale,
			final double zScale) {
		if (noiseArray == null) {
		...
		for (int j = 0; j < this.octaves; ++j) {
			...
			final double d1 = yOffset * d3 * yScale;
			double d2 = zOffset * d3 * zScale;
			long k = floor_double_long(d0);
			long l = floor_double_long(d2);
			d0 = d0 - k;
			d2 = d2 - l;
			k = k % 16777216L;
			l = l % 16777216L;
	...
	public double[] generateNoiseOctaves(final double[] noiseArray, final int xOffset, final int zOffset,
			final int xSize, final int zSize, final double xScale, final double zScale, final double p_76305_10_) {
