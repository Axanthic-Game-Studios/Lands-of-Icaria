...
public class ForgingRecipeBuilder implements RecipeBuilder {
	...
	public void save(RecipeOutput pRecipeOutput, ResourceKey<Recipe<?>> pResourceKey) {
		...
		pRecipeOutput.accept(pResourceKey, new ForgingRecipe(this.experience, this.time, this.ingredient, new ItemStack(this.result, this.amount)), builder.build(pResourceKey.identifier().withPrefix("recipes" + "/" + this.recipeCategory.getFolderName() + "/")));
	}
