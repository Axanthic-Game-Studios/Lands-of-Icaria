...
public class GrillingRecipeBuilder implements RecipeBuilder {
	...
	public void save(RecipeOutput pRecipeOutput, ResourceKey<Recipe<?>> pResourceKey) {
		...
		pRecipeOutput.accept(pResourceKey, new GrillingRecipe(this.time, this.ingredient, new ItemStack(this.result, this.amount)), builder.build(pResourceKey.location().withPrefix("recipes" + "/" + this.recipeCategory.getFolderName() + "/")));
	}
