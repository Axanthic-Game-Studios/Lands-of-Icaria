...
public class CrystalBlockRenderer implements BlockEntityRenderer<CrystalBlockEntity> {
	...
	private static void vertexA(VertexConsumer vertexConsumer, Matrix4f matrix4f, int r, int g, int b, int alpha) {
		vertexConsumer.vertex(matrix4f, 0.0F, 0.0F, 0.0F).color(r, g, b, alpha).endVertex();
	}

	private static void vertexB(VertexConsumer vertexConsumer, Matrix4f matrix4f, float length, float width) {
		vertexConsumer.vertex(matrix4f, -HALF_SQRT_3 * width, length, -0.5F * width).color(255, 255, 255, 0).endVertex();
	}

	private static void vertexC(VertexConsumer vertexConsumer, Matrix4f matrix4f, float length, float width) {
		vertexConsumer.vertex(matrix4f, HALF_SQRT_3 * width, length, -0.5F * width).color(255, 255, 255, 0).endVertex();
	}

	private static void vertexD(VertexConsumer vertexConsumer, Matrix4f matrix4f, float length, float width) {
