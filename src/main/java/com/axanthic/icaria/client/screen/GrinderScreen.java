...
public class GrinderScreen extends AbstractContainerScreen<GrinderMenu> {
	...
	public void renderBg(GuiGraphics pGuiGraphics, float pPartialTick, int pMouseX, int pMouseY) {
		...
		pGuiGraphics.blit(RenderType::guiTextured, IcariaResourceLocations.GRINDER, x, y, 0, 0, this.imageWidth, this.imageHeight, 256, 256);
	...
	public void renderFuel(GuiGraphics pGuiGraphics, int pX, int pY) {
		...
		if (maxFuel != 0) {
			...
			pGuiGraphics.blit(RenderType::guiTextured, IcariaResourceLocations.GRINDER, 82 + pX, 24 + pY + height, this.imageWidth, 16 + height, 4, fuelHeight, 256, 256);
		}
	}
	...
	public void renderProgress(GuiGraphics pGuiGraphics, int pX, int pY) {
		...
		if (maxProgress != 0) {
			...
			pGuiGraphics.blit(RenderType::guiTextured, IcariaResourceLocations.GRINDER, 95 + pX, 22 + pY, this.imageWidth, 0, width, 16, 256, 256);
		}
	}
}
