...
public class IcariaSignBlockRenderer extends SignRenderer {
	...
	public boolean isOutlineVisible(SignBlockEntity pBlockEntity, int pTextColor) {
		if (pTextColor == DyeColor.BLACK.getTextColor()) {
			...
			if (localPlayer != null && minecraft.options.getCameraType().isFirstPerson() && localPlayer.isScoping()) {
				...
				return entity != null && entity.distanceToSqr(Vec3.atCenterOf(pBlockEntity.getBlockPos())) < Mth.square(16);
			}
		}
	}
	...
	public void render(SignBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight, int pPackedOverlay) {
		...
		var woodType = SignBlock.getWoodType(blockState.getBlock());
		var material = Sheets.getSignMaterial(woodType);
		var signModel = this.map.get(woodType);
		var vertexConsumer = material.buffer(pBufferSource, signModel::renderType);
		var formattedCharSequences = pBlockEntity.getRenderMessages(Minecraft.getInstance().isTextFilteringEnabled(), (pComponent) -> {
			var list = this.font.split(pComponent, 90);
		...
		if (blockState.getBlock() instanceof StandingSignBlock) {
			...
			float f0 = -((blockState.getValue(BlockStateProperties.ROTATION_16) * 360) / 16.0F);
			...
			if (blockState.getValue(BlockStateProperties.ATTACH_FACE) == AttachFace.WALL) {
				pPoseStack.translate(0.5D, 0.5D, 0.5D);
				float f1 = -blockState.getValue(BlockStateProperties.HORIZONTAL_FACING).toYRot();
			...
			if (blockState.getValue(BlockStateProperties.ATTACH_FACE) == AttachFace.CEILING) {
				pPoseStack.translate(0.5D, 0.5D, 0.5D);
				float f2 = -blockState.getValue(BlockStateProperties.HORIZONTAL_FACING).getOpposite().toYRot();
			...
			if (blockState.getValue(BlockStateProperties.ATTACH_FACE) == AttachFace.FLOOR) {
				pPoseStack.translate(0.5D, 0.5D, 0.5D);
				float f3 = -blockState.getValue(BlockStateProperties.HORIZONTAL_FACING).getOpposite().toYRot();
		...
		for (int m = 0; m < 4; ++m) {
			var formattedCharSequence = formattedCharSequences[m];
