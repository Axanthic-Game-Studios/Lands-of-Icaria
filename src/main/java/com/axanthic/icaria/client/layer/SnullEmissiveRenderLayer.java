...
public class SnullEmissiveRenderLayer extends RenderLayer<SnullRenderState, SnullModel> {
	...
	public void submit(PoseStack poseStack, SubmitNodeCollector nodeCollector, int packedLight, SnullRenderState pRenderState, float yRot, float xRot) {
		...
		nodeCollector.submitModel(this.getParentModel(), pRenderState, poseStack, IcariaRenderTypes.SNULL_EMISSIVE, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY, IcariaClientHelper.getColorAndAlpha(pRenderState.livingEntity, pRenderState.partialTick), null, pRenderState.outlineColor, null);
		if (pRenderState.livingEntity.getCustomName() != null && pRenderState.livingEntity.getCustomName().getString().equalsIgnoreCase("Pro")) {
			nodeCollector.submitModel(this.getParentModel(), pRenderState, poseStack, IcariaRenderTypes.SNULL_EYES, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY, IcariaClientHelper.getColorAndAlpha(pRenderState.livingEntity, pRenderState.partialTick, color.getRed() / 255.0F, color.getGreen() / 255.0F, color.getBlue() / 255.0F), null, pRenderState.outlineColor, null);
			...
			nodeCollector.submitModel(this.getParentModel(), pRenderState, poseStack, IcariaRenderTypes.SNULL_SANS, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY, IcariaClientHelper.getColorAndAlpha(pRenderState.livingEntity, pRenderState.partialTick), null, pRenderState.outlineColor, null);
		}
	}
}
