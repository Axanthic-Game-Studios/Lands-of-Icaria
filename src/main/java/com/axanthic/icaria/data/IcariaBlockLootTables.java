...
public class IcariaBlockLootTables extends BlockLoot {
	...
	public void addTables() {
		...
		this.dropBone(IcariaBlocks.MARL_ROTTEN_BONES.get(), Items.BONE, IcariaItems.ROTTEN_BONES.get(), IcariaItems.REMAINS.get());
		...
		for (RegistryObject<? extends Block> registryObject : IcariaBlocks.BASIC_BLOCKS) {
			this.dropThis(registryObject.get());
		}

		for (IcariaItems.StoneDecoItemBlocks deco : IcariaItems.STONE_BLOCKS) {
			if (deco.slab != null) {
				this.add(Objects.requireNonNull(deco.block.slab).get(), BlockLoot::createSlabItemTable);
			}
			if (deco.stairs != null) {
				this.dropThis(Objects.requireNonNull(deco.block.stairs).get());
			}
			if (deco.wall != null) {
				this.dropThis(Objects.requireNonNull(deco.block.wall).get());
			}
		}
		...
		for (IcariaItems.WoodDecoItemBlocks deco : IcariaItems.WOOD_BLOCKS) {
			...
			if (deco.fence != null) {
				this.dropThis(Objects.requireNonNull(deco.block.fence).get());
			}
			if (deco.gate != null) {
				this.dropThis(Objects.requireNonNull(deco.block.gate).get());
			}
		}
	}
